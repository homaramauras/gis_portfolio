...
```{r}
colnames(confessional_df)
colnames(acled_df)
any(is.na(confessional_df$adm1)) # Check for NA in confessional_df
any(is.na(acled_df$adm1))     
any(duplicated(colnames(confessional_df)))
any(duplicated(colnames(acled_df)))
...
ggplot(merged_df, aes(x = admin1, y = total_sect, fill = sect)) +
...
Key Observations:
      Major Population Groups by Region:
          ...
          Other:Represents a smaller group distributed across regions, with peaks in Beirut.
...
North and South: Dominated by Sunni populations.

Largest Populations: Sunni in South and North regions stand out as the largest population groups across all administrative regions.

Regional Comparisons:Regions like South and North have higher overall population sizes compared to others like Akkar or Baalbek-Hermel, which show smaller populations across sects.
...
Shia regions suffer frequent attacks due to Hezbollah's presence.
Druze bear the highest casualty normalization, indicating vulnerabilities in smaller, insular communities.
Christians, especially Maronites, enjoy relative immunity due to strategic political alliances.

Policy and Humanitarian Impacts: 
Aid efforts should prioritize high-risk Shia areas (e.g., Al Nabatieh, South Lebanon) but consider Druze-specific vulnerability. Casualty mitigation in Sunni and Maronite areas may require policy efforts to reduce broader socio-political risks.

Political and Social Dynamics: 
Shia-dominant regions face military interventions targeting Hezbollah.
Christians maintain political stability but rely on international alliances for long-term security. 
Druze communities need socio-political reforms to address vulnerabilities.
...
Shia areas bear the majority of attacks but show low normalized casualties.
Druze face higher casualty normalization, underscoring systemic vulnerabilities.
Attack types (e.g., air/drone strikes) vary significantly by identity group.
...
Engage international bodies to mediate ethno-religious tensions.
