...
```{r datasets}
acled_df <- read.csv('/Users/homi/Data Science/datasets/acled_aggregated_20060.csv')
...
acled_df <- acled_df %>% select(admin1, event.month, sub.event.type, events)
...
acled_df <- acled_df %>% select(adm1, prior_month, event_type, events)
```
...
attack_map <- ggplot(combined_summary_sf) + 
  ...
  scale_fill_viridis_c(option = "viridis", name = "Total Attacks") +
